...
function ProjectCard({ projectName, apiKey, userFields, userSchema, snackBarState, snackBarMessage, popSnackBar }) {
  ...
  return (
    <div className='flex flex-col'>
      ...
      <div className='w-full p-4 bg-blue flex justify-between items-center'>
        <h1>{projectName}</h1>
        <div className='p-3 bg-white'><FaListUl /></div>
      </div>
        ...
        <div className='flex items-center gap-1'>
          ...
          <div className='flex flex-wrap gap-2'>
            { userFields && userFields.map((field) => {
              return (
                <div className='p-2 bg-white rounded-md text-md'>{field}</div>
              );
            }) }
          </div>
        </div>
